...
function FixturesScreen({ onBack, embedded }) {
  ...
  const nextRound = (FIXTURES.find(f => f.s === 'u') || {}).round;
  ...
  const controls = (
    <div style={{ padding: embedded ? '0 0 12px' : '16px 16px 12px', borderBottom:'1px solid #1e3a5f', flexShrink:0 }}>
      {!embedded && <button onClick={onBack} style={S.backBtn}>← Back</button>}
    ...
    <div style={S.page}>
      <div style={{...S.card, padding:0, display:'flex', flexDirection:'column', maxHeight:'100vh'}}>
        <div style={{padding:'16px 16px 10px', borderBottom:'1px solid #1e3a5f', flexShrink:0}}>
          ...
          <h2 style={{margin:'8px 0 2px', fontSize:18, fontWeight:800, color:'#f1f5f9'}}>{opponent}</h2>
...
function computeExtrapolatedLadder(scores) {
  ...
  const pastFixtures = FIXTURES.filter(f => {
    if (f.s === 'p') return false; // hardcoded postponed, skip
    ...
    <div style={S.page}>
      <div style={{ ...S.card, padding:0, display:'flex', flexDirection:'column', maxHeight:'100vh' }}>
        <div style={{ padding:'16px 16px 12px', borderBottom:'1px solid #1e3a5f', flexShrink:0 }}>
          <button onClick={onBack} style={S.backBtn}>← Back</button>
          <h2 style={{ margin:'8px 0 4px', fontSize:18, fontWeight:800, color:'#f1f5f9' }}>🏅 Ladder — 2026 Season</h2>
        </div>
  ...
  if (subScreen === 'fixtures') return (
    <div style={S.page}>
      <div style={{ ...S.card, padding:0, display:'flex', flexDirection:'column', maxHeight:'100vh' }}>
        <div style={{ padding:'14px 16px 10px', borderBottom:'1px solid #1e3a5f', flexShrink:0 }}>
          <button onClick={() => setSubScreen(null)} style={S.backBtn}>← Season</button>
          <h2 style={{ margin:'8px 0 0', fontSize:18, fontWeight:800, color:'#f1f5f9' }}>📅 Fixtures</h2>
        </div>
  ...
  if (subScreen === 'ladder') return (
    <div style={S.page}>
      <div style={{ ...S.card, padding:0, display:'flex', flexDirection:'column', maxHeight:'100vh' }}>
        <div style={{ padding:'14px 16px 10px', borderBottom:'1px solid #1e3a5f', flexShrink:0 }}>
          <button onClick={() => setSubScreen(null)} style={S.backBtn}>← Season</button>
          <h2 style={{ margin:'8px 0 0', fontSize:18, fontWeight:800, color:'#f1f5f9' }}>🏅 Ladder</h2>
        </div>
  ...
  if (subScreen === 'teams') return (
    <div style={S.page}>
      <div style={{ ...S.card, padding:0, display:'flex', flexDirection:'column', maxHeight:'100vh' }}>
        <div style={{ padding:'14px 16px 10px', borderBottom:'1px solid #1e3a5f', flexShrink:0 }}>
          <button onClick={() => setSubScreen(null)} style={S.backBtn}>← Season</button>
          <h2 style={{ margin:'8px 0 0', fontSize:18, fontWeight:800, color:'#f1f5f9' }}>👥 Teams</h2>
        </div>
...
export default function App() {
  ...
  if(screen==="squad")         return <SquadScreen mode={squadMode} onNext={(s,c,opp)=>{setSquad(s);setConfig(c);setOpponent(opp);setScreen("picker");}} onBack={()=>setScreen(squadBackTo)} onViewOpponent={t=>{setScoutTeam(t);setScreen("opponentStats");}} />;
