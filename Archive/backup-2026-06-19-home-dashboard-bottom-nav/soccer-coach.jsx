...
function FixturesScreen({ onBack, embedded }) {
  ...
  const tabBtn = (active) => ({
    ...
    fontWeight: 600, cursor: 'pointer', background: active ? '#166534' : '#1e293b', color: active ? '#fff' : '#94a3b8',
  });

  const numInp = { width: 56, padding: '10px 0', textAlign: 'center', fontSize: 24, fontWeight: 700,
    borderRadius: 8, border: '1px solid #334155', background: '#0f172a', color: '#f1f5f9' };

  const modal = editing && (
    <div style={{ position:'fixed', inset:0, background:'rgba(0,0,0,0.7)', zIndex:200, display:'flex', alignItems:'center', justifyContent:'center', padding:16 }}>
      <div style={{ background:'#1e293b', borderRadius:14, padding:20, width:'100%', maxWidth:340 }}>
        ...
        <button onClick={saveScore} disabled={hg===''||ag===''} style={{ width:'100%', padding:'11px 0', borderRadius:9, border:'none', fontSize:15, fontWeight:700, cursor:'pointer', background:'#166534', color:'#fff', marginBottom:8, opacity:hg===''||ag===''?0.5:1 }}>Save Score</button>
        ...
        {scores[fixtureKey(editing)] && (
          <button onClick={clearScore} style={{ width:'100%', padding:'9px 0', borderRadius:9, border:'1px solid #334155', fontSize:13, fontWeight:600, cursor:'pointer', background:'transparent', color:'#f87171', marginBottom:8 }}>Clear</button>
        )}
        <button onClick={()=>setEditing(null)} style={{ width:'100%', padding:'9px 0', borderRadius:9, border:'1px solid #334155', fontSize:13, fontWeight:600, cursor:'pointer', background:'transparent', color:'#94a3b8' }}>Cancel</button>
      </div>
    </div>
  );

  const controls = (
    <div style={{ padding: embedded ? '0 0 12px' : '16px 16px 12px', borderBottom:'1px solid #1e3a5f', flexShrink:0 }}>
      ...
      <select value={myTeam} onChange={e=>pickTeam(e.target.value)}
        style={{ width:'100%', padding:'8px 10px', borderRadius:8, background:'#0f172a', color:'#f1f5f9', border:'1px solid #334155', fontSize:13, marginBottom:10 }}>
        ...
        return (
          <div key={rnd.round}>
            <div style={{ display:'flex', alignItems:'center', gap:8, padding:'14px 0 6px' }}>
              <span style={{ fontSize:11, fontWeight:700, color:isNext?'#22c55e':'#475569', textTransform:'uppercase', letterSpacing:1 }}>{rnd.round}</span>
              {isNext && <span style={{ fontSize:10, background:'#14532d', color:'#86efac', borderRadius:4, padding:'2px 6px', fontWeight:700 }}>NEXT UP</span>}
            </div>
            {rnd.fixtures.map((f, i) => {
              ...
              if (sc?.postponed) {
                ...
                const hl = sc.home > sc.away ? '#4ade80' : sc.home < sc.away ? '#f87171' : '#94a3b8';
                const al = sc.away > sc.home ? '#4ade80' : sc.away < sc.home ? '#f87171' : '#94a3b8';
              ...
              return (
                <div key={i} onClick={() => canEdit && openScore(f)}
                  style={{ background: ours ? '#0b2818' : '#1a2332', border:`1px solid ${ours?'#22c55e55':isNext?'#3b82f633':'#1e3a5f'}`, borderRadius:10, padding:'10px 12px', marginBottom:6, cursor: canEdit ? 'pointer' : 'default' }}>
                  ...
                  <div style={{ fontSize:13, fontWeight:600, lineHeight:1.5 }}>
                    <span style={{ color:homeIsUs?'#4ade80':'#e2e8f0' }}>{f.home}</span>
                    <span style={{ color:'#475569', fontWeight:400, margin:'0 6px', fontSize:11 }}>vs</span>
                    <span style={{ color:awayIsUs?'#4ade80':'#e2e8f0' }}>{f.away}</span>
                  </div>
                  <div style={{ fontSize:11, color:'#475569', marginTop:3 }}>📍 {f.venue}{canEdit ? <span style={{color:'#334155'}}> · tap to record score</span> : ''}</div>
                </div>
              );
            })}
          </div>
  ...
  return (
    <div style={S.page}>
      <div style={{ ...S.card, padding: 0, display: 'flex', flexDirection: 'column', maxHeight:'calc(100vh - max(env(safe-area-inset-top),16px) - max(env(safe-area-inset-bottom),16px) - 32px)' }}>
        ...
        {!embedded && <div style={{borderTop:"1px solid #1e3a5f",padding:"10px 14px",paddingBottom:"max(10px,env(safe-area-inset-bottom))",flexShrink:0}}><button onClick={onBack} style={S.backBtn}>← Back</button></div>}
      </div>
    </div>
  );
}
...
function OpponentStatsScreen({ opponent, onBack, embedded, teamNotes }) {
  ...
  const statBox = (label, val, color='#e2e8f0') => (
    <div style={{flex:1, background:'#1e293b', borderRadius:8, padding:'8px 4px', textAlign:'center'}}>
      <div style={{fontSize:18, fontWeight:800, color}}>{val}</div>
      <div style={{fontSize:10, color:'#64748b', marginTop:2}}>{label}</div>
    </div>
  );

  const resultBadge = (res) => ({W:'#4ade80',L:'#f87171',D:'#fcd34d','?':'#475569'}[res]||'#475569');
    ...
    <div style={{ padding: embedded ? '0' : '14px 16px 36px' }}>
      {p > 0 ? (
        <>
          <div style={{display:'flex', gap:6, marginBottom:6}}>
            {statBox('P', p, '#94a3b8')}
            {statBox('W', w, w>0?'#4ade80':'#64748b')}
            {statBox('D', d, d>0?'#fcd34d':'#64748b')}
            {statBox('L', l, l>0?'#f87171':'#64748b')}
            {statBox('Pts', pts, '#e2e8f0')}
          </div>
          <div style={{display:'flex', gap:6, marginBottom:16}}>
            {statBox('GF', gf, '#4ade80')}
            {statBox('GA', ga, '#f87171')}
            {statBox('GD', (gd>0?'+':'')+gd, gd>0?'#4ade80':gd<0?'#f87171':'#94a3b8')}
            {statBox('Ladder', theirPos>0?`#${theirPos}`:'–', '#c4b5fd')}
          </div>
        </>
      ) : (
        <div style={{background:'#1e293b', borderRadius:10, padding:'12px 14px', marginBottom:16, fontSize:13, color:'#64748b'}}>
      ...
      <div style={{background:'#0a1929', border:'1px solid #3b82f644', borderRadius:12, padding:'14px', marginBottom:16}}>
        <div style={{display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:10}}>
          ...
          <button
            ...
            style={{background:'#1e3a5f', border:'1px solid #3b82f633', color:'#93c5fd', borderRadius:6, padding:'3px 9px', fontSize:11, cursor:'pointer'}}
          >↻ Refresh</button>
        </div>
      ...
      {uqfcGames.length > 0 && (
        <div style={{marginBottom:16}}>
          ...
          {uqfcGames.map((r,i)=>(
            <div key={i} style={{background:'#1e293b', borderRadius:9, padding:'10px 12px', marginBottom:6, display:'flex', justifyContent:'space-between', alignItems:'center'}}>
              <div>
      ...
      {results.length > 0 && (
        <div>
          ...
          {results.map((r,i)=>(
            <div key={i} style={{background:'#111827', borderRadius:8, padding:'8px 12px', marginBottom:4, display:'flex', justifyContent:'space-between', alignItems:'center'}}>
              <div>
  ...
  return (
    <div style={S.page}>
      <div style={{...S.card, padding:0, display:'flex', flexDirection:'column', maxHeight:'calc(100vh - max(env(safe-area-inset-top),16px) - max(env(safe-area-inset-bottom),16px) - 32px)'}}>
        <div style={{padding:'16px 16px 10px', borderBottom:'1px solid #1e3a5f', flexShrink:0}}>
        ...
        <div style={{borderTop:"1px solid #1e3a5f",padding:"10px 14px",paddingBottom:"max(10px,env(safe-area-inset-bottom))",flexShrink:0}}><button onClick={onBack} style={S.backBtn}>← Back</button></div>
      </div>
    </div>
  );
}
...
function LadderScreen({ onBack, embedded }) {
  ...
  const cell = (content, style={}) => (
    <td style={{ padding:'8px 5px', textAlign:'center', fontSize:12, borderBottom:'1px solid #1e3a5f', ...style }}>{content}</td>
  );
        ...
        <thead>
          <tr style={{ background:'#0a0f1a' }}>
            {['#','Team','P','W','D','L','GF','GA','GD','Pts'].map(h=>(
              <th key={h} style={{ padding:'7px 5px', fontSize:10, fontWeight:700, color:'#64748b', textTransform:'uppercase', letterSpacing:0.5, textAlign:h==='Team'?'left':'center', borderBottom:'1px solid #1e3a5f', whiteSpace:'nowrap' }}>{h}</th>
            ))}
          </tr>
        </thead>
        <tbody>
          {rows.map((t, i) => {
            ...
            const rowBg = isUs ? '#0b2818' : hasAssumptions ? '#16120a' : i%2===0 ? '#111827' : '#0d1420';
            return (
              <tr key={t.name} style={{ background:rowBg }}>
                <td style={{ padding:'8px 5px', textAlign:'center', fontSize:12, borderBottom:'1px solid #1e3a5f', color:'#64748b', fontWeight:700 }}>{i+1}</td>
                <td style={{ padding:'8px 7px', fontSize:12, borderBottom:'1px solid #1e3a5f', color:isUs?'#4ade80':'#e2e8f0', fontWeight:isUs?700:400, whiteSpace:'nowrap', maxWidth:130, overflow:'hidden', textOverflow:'ellipsis' }}>
                  {isUs && '⭐ '}{t.name}
                  {hasAssumptions && <span style={{fontSize:9,color:'#f59e0b',marginLeft:3,fontWeight:700}}>~{t.assumedPts}est</span>}
                </td>
                {cell(t.p, {color:'#94a3b8'})}
                {cell(t.w, {color:t.w>0?'#4ade80':'#64748b'})}
                {cell(t.d, {color:t.d>0?'#fcd34d':'#64748b'})}
                {cell(t.l, {color:t.l>0?'#f87171':'#64748b'})}
                {cell(t.gf, {color:'#94a3b8'})}
                {cell(t.ga, {color:'#94a3b8'})}
                {cell((gd>0?'+':'')+gd, {color:gd>0?'#4ade80':gd<0?'#f87171':'#94a3b8'})}
  ...
  const toggleBar = (
    <div style={{ display:'flex', gap:6, margin:'10px 0 12px' }}>
      <button onClick={()=>setView('current')} style={{
        ...
        background: view==='current' ? '#166534' : '#1e293b',
        color: view==='current' ? '#fff' : '#94a3b8',
      }}>📊 Current</button>
      <button onClick={()=>setView('extrapolated')} style={{
        flex:1, padding:'8px 0', borderRadius:8, border:'none', fontSize:13, fontWeight:700, cursor:'pointer',
        background: view==='extrapolated' ? '#78350f' : '#1e293b',
  ...
  return (
    <div style={S.page}>
      <div style={{ ...S.card, padding:0, display:'flex', flexDirection:'column', maxHeight:'calc(100vh - max(env(safe-area-inset-top),16px) - max(env(safe-area-inset-bottom),16px) - 32px)' }}>
        <div style={{ padding:'16px 16px 12px', borderBottom:'1px solid #1e3a5f', flexShrink:0 }}>
  ...
  if (subScreen === 'log') {
    ...
    <div style={S.page}>
      <div style={{ ...S.card, padding:0, display:'flex', flexDirection:'column', maxHeight:'calc(100vh - max(env(safe-area-inset-top),16px) - max(env(safe-area-inset-bottom),16px) - 32px)' }}>
        ...
        <div style={{borderTop:"1px solid #1e3a5f",padding:"10px 14px",paddingBottom:"max(10px,env(safe-area-inset-bottom))",flexShrink:0}}><button onClick={()=>setSubScreen(null)} style={S.backBtn}>← Season</button></div>
      </div>
    </div>
  );}
      ...
      <div style={{ ...S.card, padding:0, display:'flex', flexDirection:'column', maxHeight:'calc(100vh - max(env(safe-area-inset-top),16px) - max(env(safe-area-inset-bottom),16px) - 32px)' }}>
        ...
        <div style={{ overflowY:'auto', flex:1, padding:'14px 16px 40px' }}>
          <select value={selectedTeam} onChange={e => selectTeam(e.target.value)}
            style={{ width:'100%', padding:'8px 10px', borderRadius:8, background:'#0f172a', color:'#f1f5f9', border:'1px solid #334155', fontSize:13, marginBottom:14 }}>
            ...
            <>
              ...
              <div style={{ marginTop:16, borderTop:'1px solid #1e3a5f', paddingTop:16 }}>
                ...
                <textarea value={noteText} onChange={e => setNoteText(e.target.value)}
                  ...
                  style={{ width:'100%', minHeight:90, padding:'10px 12px', background:'#0f172a', border:'1px solid #334155', borderRadius:10, color:'#f1f5f9', fontSize:13, lineHeight:1.6, resize:'vertical', fontFamily:'inherit', outline:'none' }}
                />
    ...
    <div style={S.page}>
      <div style={S.card}>
        ...
        <div style={S.tileGrid}>
          <button style={{ ...S.tile, background:'#0f172a', border:'1px solid #1e3a5f' }} onClick={() => setSubScreen('log')}>
          ...
          <button style={{ ...S.tile, background:'#0f172a', border:'1px solid #1e3a5f' }} onClick={() => setSubScreen('fixtures')}>
            <span style={S.tileIcon}>📅</span>
            <span style={S.tileLbl}>Fixtures</span>
          </button>
          <button style={{ ...S.tile, background:'#0f172a', border:'1px solid #1e3a5f' }} onClick={() => setSubScreen('ladder')}>
            <span style={S.tileIcon}>🏅</span>
            <span style={S.tileLbl}>Ladder</span>
          </button>
          <button style={{ ...S.tile, background:'#0f172a', border:'1px solid #1e3a5f' }} onClick={() => setSubScreen('teams')}>
            <span style={S.tileIcon}>👥</span>
            <span style={S.tileLbl}>Teams</span>
          </button>
        </div>

        <div style={{borderTop:"1px solid #1e3a5f",padding:"10px 14px",paddingBottom:"max(10px,env(safe-area-inset-bottom))",flexShrink:0}}><button onClick={onBack} style={S.backBtn}>← Menu</button></div>
      </div>
    </div>
  );
}
          ...
          return (
            <div>
              ...
              {all.length>0 && (
                <div style={{marginBottom:16}}>
                  <div style={{display:"flex",justifyContent:"space-between",marginBottom:8}}>
                    <span style={{fontSize:12,fontWeight:700,color:"#4ade80"}}>⭐ Us</span>
                  ...
                  {MATCH_EVENT_RULES.filter(r=>r.type!=='NOTE').map(rule=>{
                    ...
                    return (<div key={rule.type} style={{marginBottom:10}}>
                      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:3}}>
                        <span style={{fontSize:13,fontWeight:800,color:"#4ade80",minWidth:20}}>{us}</span>
                      ...
                      <div style={{position:"relative",height:5,background:"#1e293b",borderRadius:3}}>
                        <div style={{position:"absolute",right:"50%",top:0,height:"100%",width:(us/maxSt*50)+"%",background:"#22c55e",borderRadius:"3px 0 0 3px"}}/>
              ...
              {all.map(ev=>{
                ...
                return (<div key={ev.id} style={{background:"#0f172a",borderLeft:"3px solid "+rule.color,borderRadius:"0 8px 8px 0",padding:"7px 10px",marginBottom:5,display:"flex",alignItems:"flex-start",gap:8}}>
                  ...
                  <div style={{flex:1}}>
                    <div style={{display:"flex",alignItems:"center",gap:5,flexWrap:"wrap"}}>
                      ...
                      {ev.team&&<span style={{fontSize:10,color:ev.team==="us"?"#4ade80":"#fca5a5",background:ev.team==="us"?"#0b281833":"#7f1d1d33",borderRadius:4,padding:"1px 5px"}}>{ev.team==="us"?"Us":game.opponent||"Them"}</span>}
                    </div>
  ...
  return (
    <div style={S.page}>
      <div style={S.card}>
        ...
        {isNewGame && <div style={{borderTop:"1px solid #1e3a5f",paddingTop:10,marginTop:4}}><button style={S.btnGhost} onClick={onBack}>← Back</button></div>}
      </div>
    </div>
  );
}
...
const MATCH_EVENT_RULES = [
  ...
  { type:"SUBSTITUTION", icon:"🔄", color:"#86efac", label:"Sub"              },
  ...
  return (
    <div style={{...S.wrap,padding:0,background:BG}}>
      ...
      <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',padding:'10px 16px',background:BG,borderBottom:'1px solid '+CARD,flexShrink:0,width:'100%',maxWidth:500}}>
        <button style={{...S.btnGhost,borderColor:'#2a4a2a',color:'#6a9a6a'}} onClick={onBack}>← Pitch</button>
        <div style={{fontSize:14,fontWeight:800,color:'#f1f5f9'}}>{usGoalsLen} – {themGoalsLen} <span style={{fontSize:11,color:'#6a9a6a',fontWeight:400}}>{mm}:{ss}</span></div>
        <div style={{width:60}}/>
      </div>
      ...
      <div style={{flex:1,overflowY:'auto',padding:'16px',background:BG,width:'100%',maxWidth:500,boxSizing:'border-box'}}>
        ...
        {evTab==='record' && (
          <div>
            {pendingType && (()=>{
              ...
              return (
                <div style={{background:CARD,border:'2px solid '+rule.color+'55',borderRadius:12,padding:14,marginBottom:14}}>
                  ...
                  {pendingType==='NOTE' ? (
                    <div>
                      <textarea value={noteText} onChange={e=>setNoteText(e.target.value)} rows={2} placeholder="What happened?" style={{...S.inp,resize:'none',width:'100%',marginBottom:8,fontSize:13,background:'#0d200d',borderColor:'#2a4a2a'}}/>
                      <div style={{display:'flex',gap:6,marginBottom:8}}>
                        <button onClick={()=>addEv('NOTE','us',{notes:noteText})} style={{flex:1,padding:'11px 6px',borderRadius:9,border:'none',background:'#0b3a1a',color:'#86efac',fontWeight:700,fontSize:13,cursor:'pointer'}}>⭐ {OUR}</button>
                      ...
                      <button onClick={()=>setPendingType(null)} style={{width:'100%',padding:'9px',borderRadius:9,border:'1px solid #2a4a2a',background:'transparent',color:'#6a9a6a',fontSize:12,cursor:'pointer'}}>Cancel</button>
                    </div>
                  ) : (
                    <div>
                      <div style={{display:'flex',gap:8,marginBottom:8}}>
                        <button onClick={()=>addEv(pendingType,'us')} style={{flex:1,padding:'14px 8px',borderRadius:9,border:'none',background:'#0b3a1a',color:'#86efac',fontWeight:700,fontSize:15,cursor:'pointer'}}>⭐ {OUR}</button>
            ...
            {statusMsg&&<div style={{background:'#0b2818',border:'1px solid #22c55e44',borderRadius:9,padding:'9px 12px',marginBottom:12}}><span style={{fontSize:13,color:'#4ade80',fontWeight:600}}>{statusMsg}</span></div>}
            ...
            <div style={{borderTop:'1px solid '+CARD,paddingTop:14}}>
              <div style={{fontSize:11,fontWeight:700,color:'#6a9a6a',textTransform:'uppercase',letterSpacing:1,marginBottom:8}}>🤖 Describe what happened</div>
              <textarea value={aiText} onChange={e=>setAiText(e.target.value)} rows={3}
                placeholder='e.g. "Corner for us, keeper tipped it over"'
                style={{...S.inp,resize:'none',width:'100%',lineHeight:1.6,fontSize:13,background:'#0d200d',borderColor:'#2a4a2a'}}/>
        ...
        {evTab==='stats' && (
          <div>
            ...
            <div style={{background:CARD,borderRadius:12,padding:'20px 16px',marginBottom:20,textAlign:'center'}}>
              <div style={{fontSize:11,fontWeight:700,color:'#6a9a6a',letterSpacing:2,textTransform:'uppercase',marginBottom:10}}>FINAL SCORE</div>
                ...
                return (
                  <div key={rule.type} style={{marginBottom:14}}>
                    ...
                    <div style={{height:4,background:BORDER,borderRadius:2,position:'relative'}}>
                      ...
                      <div style={{position:'absolute',left:'50%',top:0,height:'100%',width:1,background:'#2a4a2a'}}/>
...
function BottomNav({ activeTab, onTab }) {
  ...
  return (
    <nav style={{
      ...
      background:'#0d1526', borderTop:'1px solid #1e3a5f',
      ...
      {tabs.map(t => {
        ...
        return (
          <button key={t.id} onClick={() => onTab(t.id)} style={{
            ...
            padding:'4px 8px', color: active ? '#22c55e' : '#64748b',
            minWidth:56,
          }}>
            <span style={{display:'flex', alignItems:'center', justifyContent:'center', width:22, height:22}}>
              {React.cloneElement(t.svg, { stroke: active ? '#22c55e' : '#64748b' })}
            </span>
...
function HomeScreen({ games, settings, onMatchDay, onGoSeason, onGoTeam }) {
  ...
  return (
    <div style={{ minHeight:'100vh', background:'#0a0e1a', paddingBottom:80, paddingTop:'max(env(safe-area-inset-top),0px)' }}>
      ...
      <div style={{ padding:'20px 20px 16px', display:'flex', alignItems:'center', justifyContent:'space-between' }}>
        ...
        <button onClick={onGoTeam} style={{ background:'none', border:'1px solid #1e3a5f', borderRadius:10, padding:'7px 10px', cursor:'pointer', color:'#64748b' }}>
      ...
      {nextFix ? (
        <div onClick={onGoSeason} style={{
          ...
          <div style={{ padding:'16px 18px' }}>
            <div style={{ fontSize:10, fontWeight:700, color:'#86efac', letterSpacing:1.5, textTransform:'uppercase', marginBottom:10 }}>
            ...
            <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between' }}>
              <div>
                ...
                <div style={{ fontSize:12, color:'#4ade8088', marginTop:4 }}>
          ...
          <div style={{ background:'#ffffff0a', padding:'10px 18px', display:'flex', alignItems:'center', justifyContent:'space-between' }}>
            <span style={{ fontSize:12, color:'#86efac', fontWeight:600 }}>View fixtures →</span>
          </div>
        </div>
      ) : (
        <div style={{ margin:'0 16px 16px', borderRadius:16, background:'#1e293b', border:'1px solid #1e3a5f', padding:'20px 18px', textAlign:'center' }}>
      ...
      {played > 0 && (
        <div style={{ margin:'0 16px 16px' }}>
          ...
          <div style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:8 }}>
            {[['Played', played, '#f1f5f9'], ['Won', wins, '#22c55e'], ['Drawn', draws, '#f59e0b'], ['Lost', losses, '#ef4444']].map(([label, val, color]) => (
              <div key={label} style={{ background:'#131929', borderRadius:12, padding:'12px 8px', textAlign:'center', border:'1px solid #1e3a5f' }}>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:10 }}>
          {[
            ...
            <button key={item.label} onClick={item.action} style={{
              background: item.highlight ? '#22c55e' : '#131929',
              border: item.highlight ? 'none' : '1px solid #1e3a5f',
              borderRadius:14, padding:'14px 4px 10px',
              display:'flex', flexDirection:'column', alignItems:'center', gap:6,
              cursor:'pointer',
            }}>
              <span style={{ fontSize:24 }}>{item.icon}</span>
              <span style={{ fontSize:11, fontWeight:700, color: item.highlight ? '#0f172a' : '#f1f5f9', lineHeight:1.2 }}>{item.label}</span>
            </button>
          ))}
        </div>
      </div>
      ...
      {lastRes && (
        <div style={{ margin:'0 16px 0' }}>
          ...
          <div style={{ background:'#131929', borderRadius:14, padding:'14px 16px', border:'1px solid #1e3a5f', display:'flex', alignItems:'center', gap:12 }}>
            {(() => {
...
function MoreScreen({ onSettings, onImportExport }) {
  ...
  return (
    <div style={{ minHeight:'100vh', background:'#0a0e1a', paddingBottom:90, paddingTop:'max(env(safe-area-inset-top),16px)' }}>
      ...
      <div style={{ padding:'0 16px', display:'flex', flexDirection:'column', gap:10 }}>
        {items.map(item => (
          <button key={item.label} onClick={item.action} style={{
            background:'#131929', border:'1px solid #1e3a5f', borderRadius:14,
...
// ── Styles ────────────────────────────────────────────────────────────────────
const S = {
  page:         { minHeight:"100vh", background:"#0a0e1a", display:"flex", alignItems:"center", justifyContent:"center", padding:16, paddingTop:"max(env(safe-area-inset-top),16px)", paddingBottom:"max(env(safe-area-inset-bottom),16px)", boxSizing:"border-box" },
  card:         { background:"#131929", border:"1px solid #1e3a5f", borderRadius:16, padding:"24px 20px", width:"100%", maxWidth:DEVICE.cardMax, display:"flex", flexDirection:"column", gap:12 },
  ...
  backBtn:      { background:"none", border:"1px solid #334155", color:"#94a3b8", borderRadius:8, padding:"6px 10px", fontSize:13, cursor:"pointer" },
  ...
  scoreTile:    { background:"#0a0e1a", borderRadius:12, padding:"14px 16px", border:"1px solid #1e3a5f" },
  ...
  gameRow:      { display:"flex", alignItems:"center", gap:10, background:"#0a0e1a", borderRadius:10, padding:"10px 12px", cursor:"pointer" },
  ...
  factCard:     { background:"#0a0e1a", borderRadius:10, padding:"10px 12px" },
  ...
  wrap:         { minHeight:"100vh", background:"#0a0e1a", color:"#f1f5f9", fontFamily:"'Inter',system-ui,sans-serif", display:"flex", flexDirection:"column", alignItems:"center", padding:"10px 8px 24px", paddingTop:"max(env(safe-area-inset-top),10px)" },
