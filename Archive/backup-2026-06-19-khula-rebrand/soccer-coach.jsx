...
function FixturesScreen({ onBack, embedded }) {
  ...
  const tabBtn = (active) => ({
    ...
    fontWeight: 600, cursor: 'pointer', background: active ? '#7a4500' : '#1e1e1e', color: active ? '#fff' : '#94a3b8',
  });

  const numInp = { width: 56, padding: '10px 0', textAlign: 'center', fontSize: 24, fontWeight: 700,
    borderRadius: 8, border: '1px solid #2e2e2e', background: '#0D0D0D', color: '#f1f5f9' };

  const modal = editing && (
    <div style={{ position:'fixed', inset:0, background:'rgba(0,0,0,0.7)', zIndex:200, display:'flex', alignItems:'center', justifyContent:'center', padding:16 }}>
      <div style={{ background:'#1e1e1e', borderRadius:14, padding:20, width:'100%', maxWidth:340 }}>
        <div style={{ fontSize:13, color:'#64748b', marginBottom:4 }}>{editing.date} · {editing.time}</div>
        <div style={{ fontSize:13, color:'#94a3b8', marginBottom:16, lineHeight:1.4 }}>
          {editing.home} <span style={{color:'#475569'}}>vs</span> {editing.away}
        </div>
        <div style={{ display:'flex', alignItems:'center', justifyContent:'center', gap:16, marginBottom:20 }}>
          <div style={{ textAlign:'center' }}>
            <div style={{ fontSize:11, color:'#64748b', marginBottom:6 }}>HOME</div>
            <input type="number" min="0" value={hg} onChange={e=>setHg(e.target.value)} style={numInp} />
          </div>
          <div style={{ fontSize:20, color:'#475569', fontWeight:700 }}>–</div>
          <div style={{ textAlign:'center' }}>
            <div style={{ fontSize:11, color:'#64748b', marginBottom:6 }}>AWAY</div>
            <input type="number" min="0" value={ag} onChange={e=>setAg(e.target.value)} style={numInp} />
          </div>
        </div>
        <button onClick={saveScore} disabled={hg===''||ag===''} style={{ width:'100%', padding:'11px 0', borderRadius:9, border:'none', fontSize:15, fontWeight:700, cursor:'pointer', background:'#7a4500', color:'#fff', marginBottom:8, opacity:hg===''||ag===''?0.5:1 }}>Save Score</button>
        ...
        {scores[fixtureKey(editing)] && (
          <button onClick={clearScore} style={{ width:'100%', padding:'9px 0', borderRadius:9, border:'1px solid #2e2e2e', fontSize:13, fontWeight:600, cursor:'pointer', background:'transparent', color:'#f87171', marginBottom:8 }}>Clear</button>
        )}
        <button onClick={()=>setEditing(null)} style={{ width:'100%', padding:'9px 0', borderRadius:9, border:'1px solid #2e2e2e', fontSize:13, fontWeight:600, cursor:'pointer', background:'transparent', color:'#94a3b8' }}>Cancel</button>
      </div>
    </div>
  );

  const controls = (
    <div style={{ padding: embedded ? '0 0 12px' : '16px 16px 12px', borderBottom:'1px solid #2a2000', flexShrink:0 }}>
      {!embedded && <h2 style={{ margin:'8px 0 12px', fontSize:18, fontWeight:800, color:'#f1f5f9' }}>📅 Fixtures — 2026 Season</h2>}
      <select value={myTeam} onChange={e=>pickTeam(e.target.value)}
        style={{ width:'100%', padding:'8px 10px', borderRadius:8, background:'#0D0D0D', color:'#f1f5f9', border:'1px solid #2e2e2e', fontSize:13, marginBottom:10 }}>
  ...
  const list = (
    <div style={{ padding:'4px 16px 40px' }}>
      {grouped.length===0 && <div style={{ textAlign:'center', color:'#64748b', padding:'40px 0', fontSize:14 }}>No fixtures found.</div>}
      {grouped.map(rnd => {
        const isNext = rnd.round === nextRound;
        return (
          <div key={rnd.round}>
            <div style={{ display:'flex', alignItems:'center', gap:8, padding:'14px 0 6px' }}>
              <span style={{ fontSize:11, fontWeight:700, color:isNext?'#E8A020':'#475569', textTransform:'uppercase', letterSpacing:1 }}>{rnd.round}</span>
              {isNext && <span style={{ fontSize:10, background:'#4a2800', color:'#FDE68A', borderRadius:4, padding:'2px 6px', fontWeight:700 }}>NEXT UP</span>}
            </div>
            {rnd.fixtures.map((f, i) => {
              ...
              if (sc?.postponed) {
                ...
                const hl = sc.home > sc.away ? '#FCD34D' : sc.home < sc.away ? '#f87171' : '#94a3b8';
                const al = sc.away > sc.home ? '#FCD34D' : sc.away < sc.home ? '#f87171' : '#94a3b8';
                ...
                scoreBadge = <span style={{ fontSize:11, color:'#475569', fontStyle:'italic' }}>Unknown</span>;
              } else {
                scoreBadge = <span style={{ fontSize:11, color:'#60a5fa', fontWeight:500 }}>Upcoming</span>;
              }
              return (
                <div key={i} onClick={() => canEdit && openScore(f)}
                  style={{ background: ours ? '#1a1200' : '#1a2332', border:`1px solid ${ours?'#E8A02055':isNext?'#E8A02033':'#2a2000'}`, borderRadius:10, padding:'10px 12px', marginBottom:6, cursor: canEdit ? 'pointer' : 'default' }}>
                  <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:4 }}>
                    <span style={{ fontSize:12, color:'#64748b' }}>{f.date} · {f.time}</span>
                    {scoreBadge}
                  </div>
                  <div style={{ fontSize:13, fontWeight:600, lineHeight:1.5 }}>
                    <span style={{ color:homeIsUs?'#FCD34D':'#e2e8f0' }}>{f.home}</span>
                    <span style={{ color:'#475569', fontWeight:400, margin:'0 6px', fontSize:11 }}>vs</span>
                    <span style={{ color:awayIsUs?'#FCD34D':'#e2e8f0' }}>{f.away}</span>
                  </div>
                  <div style={{ fontSize:11, color:'#475569', marginTop:3 }}>📍 {f.venue}{canEdit ? <span style={{color:'#2e2e2e'}}> · tap to record score</span> : ''}</div>
                </div>
              );
            })}
          </div>
  ...
  return (
    <div style={S.page}>
      <div style={{ ...S.card, padding: 0, display: 'flex', flexDirection: 'column', maxHeight:'calc(100vh - max(env(safe-area-inset-top),16px) - max(env(safe-area-inset-bottom),16px) - 32px)' }}>
        ...
        {!embedded && <div style={{borderTop:"1px solid #2a2000",padding:"10px 14px",paddingBottom:"max(10px,env(safe-area-inset-bottom))",flexShrink:0}}><button onClick={onBack} style={S.backBtn}>← Back</button></div>}
      </div>
    </div>
  );
}
...
function OpponentStatsScreen({ opponent, onBack, embedded, teamNotes }) {
  ...
  const statBox = (label, val, color='#e2e8f0') => (
    <div style={{flex:1, background:'#1e1e1e', borderRadius:8, padding:'8px 4px', textAlign:'center'}}>
      <div style={{fontSize:18, fontWeight:800, color}}>{val}</div>
      <div style={{fontSize:10, color:'#64748b', marginTop:2}}>{label}</div>
    </div>
  );

  const resultBadge = (res) => ({W:'#FCD34D',L:'#f87171',D:'#fcd34d','?':'#475569'}[res]||'#475569');
    ...
    <div style={{ padding: embedded ? '0' : '14px 16px 36px' }}>
      {p > 0 ? (
        <>
          <div style={{display:'flex', gap:6, marginBottom:6}}>
            {statBox('P', p, '#94a3b8')}
            {statBox('W', w, w>0?'#FCD34D':'#64748b')}
            {statBox('D', d, d>0?'#fcd34d':'#64748b')}
            {statBox('L', l, l>0?'#f87171':'#64748b')}
            {statBox('Pts', pts, '#e2e8f0')}
          </div>
          <div style={{display:'flex', gap:6, marginBottom:16}}>
            {statBox('GF', gf, '#FCD34D')}
            {statBox('GA', ga, '#f87171')}
            {statBox('GD', (gd>0?'+':'')+gd, gd>0?'#FCD34D':gd<0?'#f87171':'#94a3b8')}
            {statBox('Ladder', theirPos>0?`#${theirPos}`:'–', '#c4b5fd')}
          </div>
        </>
      ) : (
        <div style={{background:'#1e1e1e', borderRadius:10, padding:'12px 14px', marginBottom:16, fontSize:13, color:'#64748b'}}>
      ...
      <div style={{background:'#0a1929', border:'1px solid #3b82f644', borderRadius:12, padding:'14px', marginBottom:16}}>
        <div style={{display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:10}}>
          ...
          <button
            ...
            style={{background:'#2a2000', border:'1px solid #E8A02033', color:'#93c5fd', borderRadius:6, padding:'3px 9px', fontSize:11, cursor:'pointer'}}
          >↻ Refresh</button>
        </div>
      ...
      {uqfcGames.length > 0 && (
        <div style={{marginBottom:16}}>
          <div style={{fontSize:11, fontWeight:700, color:'#64748b', textTransform:'uppercase', letterSpacing:1, marginBottom:8}}>vs UQFC Teams</div>
          {uqfcGames.map((r,i)=>(
            <div key={i} style={{background:'#1e1e1e', borderRadius:9, padding:'10px 12px', marginBottom:6, display:'flex', justifyContent:'space-between', alignItems:'center'}}>
              <div>
                <div style={{fontSize:13, color:'#e2e8f0'}}>{r.vs}</div>
                <div style={{fontSize:11, color:'#64748b'}}>{r.f.date} · {r.f.round}</div>
              </div>
              {r.result!=='?' ? (
              ...
              ) : <span style={{fontSize:11,color:'#475569',fontStyle:'italic'}}>Unknown</span>}
            </div>
          ))}
        </div>
      )}
      {results.length > 0 && (
        <div>
          <div style={{fontSize:11, fontWeight:700, color:'#64748b', textTransform:'uppercase', letterSpacing:1, marginBottom:8}}>All Results This Season</div>
          {results.map((r,i)=>(
            <div key={i} style={{background:'#121212', borderRadius:8, padding:'8px 12px', marginBottom:4, display:'flex', justifyContent:'space-between', alignItems:'center'}}>
              <div>
                ...
                <div style={{fontSize:10, color:'#64748b'}}>{r.f.date} · {r.f.round}</div>
              </div>
              {r.result!=='?' ? (
                <div>
  ...
  return (
    <div style={S.page}>
      <div style={{...S.card, padding:0, display:'flex', flexDirection:'column', maxHeight:'calc(100vh - max(env(safe-area-inset-top),16px) - max(env(safe-area-inset-bottom),16px) - 32px)'}}>
        <div style={{padding:'16px 16px 10px', borderBottom:'1px solid #2a2000', flexShrink:0}}>
          <h2 style={{margin:'4px 0 2px', fontSize:18, fontWeight:800, color:'#f1f5f9'}}>{opponent}</h2>
          <div style={{fontSize:12, color:'#64748b'}}>2026 Season · Scouting Report</div>
        </div>
        <div style={{overflowY:'auto', flex:1, padding:'14px 16px 36px'}}>
          {statsContent}
        </div>
        <div style={{borderTop:"1px solid #2a2000",padding:"10px 14px",paddingBottom:"max(10px,env(safe-area-inset-bottom))",flexShrink:0}}><button onClick={onBack} style={S.backBtn}>← Back</button></div>
      </div>
    </div>
  );
}
...
function LadderScreen({ onBack, embedded }) {
  ...
  const cell = (content, style={}) => (
    <td style={{ padding:'8px 5px', textAlign:'center', fontSize:12, borderBottom:'1px solid #2a2000', ...style }}>{content}</td>
  );
        ...
        <thead>
          <tr style={{ background:'#0a0f1a' }}>
            {['#','Team','P','W','D','L','GF','GA','GD','Pts'].map(h=>(
              <th key={h} style={{ padding:'7px 5px', fontSize:10, fontWeight:700, color:'#64748b', textTransform:'uppercase', letterSpacing:0.5, textAlign:h==='Team'?'left':'center', borderBottom:'1px solid #2a2000', whiteSpace:'nowrap' }}>{h}</th>
            ))}
          </tr>
        </thead>
        <tbody>
          {rows.map((t, i) => {
            ...
            const rowBg = isUs ? '#1a1200' : hasAssumptions ? '#16120a' : i%2===0 ? '#121212' : '#0d1420';
            return (
              <tr key={t.name} style={{ background:rowBg }}>
                <td style={{ padding:'8px 5px', textAlign:'center', fontSize:12, borderBottom:'1px solid #2a2000', color:'#64748b', fontWeight:700 }}>{i+1}</td>
                <td style={{ padding:'8px 7px', fontSize:12, borderBottom:'1px solid #2a2000', color:isUs?'#FCD34D':'#e2e8f0', fontWeight:isUs?700:400, whiteSpace:'nowrap', maxWidth:130, overflow:'hidden', textOverflow:'ellipsis' }}>
                  {isUs && '⭐ '}{t.name}
                  {hasAssumptions && <span style={{fontSize:9,color:'#f59e0b',marginLeft:3,fontWeight:700}}>~{t.assumedPts}est</span>}
                </td>
                {cell(t.p, {color:'#94a3b8'})}
                {cell(t.w, {color:t.w>0?'#FCD34D':'#64748b'})}
                {cell(t.d, {color:t.d>0?'#fcd34d':'#64748b'})}
                {cell(t.l, {color:t.l>0?'#f87171':'#64748b'})}
                {cell(t.gf, {color:'#94a3b8'})}
                {cell(t.ga, {color:'#94a3b8'})}
                {cell((gd>0?'+':'')+gd, {color:gd>0?'#FCD34D':gd<0?'#f87171':'#94a3b8'})}
                {cell(t.pts, {color: hasAssumptions?'#f59e0b':'#f1f5f9', fontWeight:700, fontSize:14})}
              </tr>
            );
          })}
        </tbody>
  ...
  const toggleBar = (
    <div style={{ display:'flex', gap:6, margin:'10px 0 12px' }}>
      <button onClick={()=>setView('current')} style={{
        ...
        background: view==='current' ? '#7a4500' : '#1e1e1e',
        color: view==='current' ? '#fff' : '#94a3b8',
      }}>📊 Current</button>
      <button onClick={()=>setView('extrapolated')} style={{
        flex:1, padding:'8px 0', borderRadius:8, border:'none', fontSize:13, fontWeight:700, cursor:'pointer',
        background: view==='extrapolated' ? '#78350f' : '#1e1e1e',
        color: view==='extrapolated' ? '#fcd34d' : '#94a3b8',
      }}>🔮 Extrapolated</button>
    </div>
  );
  ...
  const tableContent = (
    <>
      {!hasData && view==='current' && <p style={{ fontSize:13, color:'#64748b', margin:'8px 0' }}>No scores recorded yet. Enter results in Fixtures to see the ladder.</p>}
      ...
      {view === 'current' && (
        <>
          ...
          <div style={{ padding:'6px 0 10px', fontSize:11, color:'#475569' }}>
      ...
      {view === 'extrapolated' && (
        <>
          ...
          {extLadder.length === 0
            ? <p style={{ fontSize:13, color:'#64748b' }}>No past fixtures found to extrapolate.</p>
  ...
  return (
    <div style={S.page}>
      <div style={{ ...S.card, padding:0, display:'flex', flexDirection:'column', maxHeight:'calc(100vh - max(env(safe-area-inset-top),16px) - max(env(safe-area-inset-bottom),16px) - 32px)' }}>
        <div style={{ padding:'16px 16px 12px', borderBottom:'1px solid #2a2000', flexShrink:0 }}>
          <h2 style={{ margin:'4px 0 4px', fontSize:18, fontWeight:800, color:'#f1f5f9' }}>🏅 Ladder — 2026 Season</h2>
        </div>
  ...
  return (
    <div style={S.page}>
      <div style={S.card}>
        ...
        {/* Import / Export */}
        <div style={{borderTop:'1px solid #2a2000', paddingTop:12, marginTop:4}}>
        ...
        <div style={{borderTop:"1px solid #2a2000",paddingTop:10,marginTop:4}}>
  ...
  return (
    <div style={S.page}>
      <div style={{...S.card,maxWidth:440,padding:0}}>
        <div style={{padding:"14px 16px 10px",borderBottom:"1px solid #2a2000"}}>
        ...
        {rows.length===0
          ...
          : <>
              {/* Header */}
              <div style={{display:"flex",alignItems:"center",padding:"8px 16px",background:"#0D0D0D",borderBottom:"1px solid #2a2000"}}>
              ...
              {rows.map((r,i)=>(
                <div key={r.name} style={{display:"flex",alignItems:"center",padding:"10px 16px",borderBottom:"1px solid #0D0D0D",background:i%2===0?"#121212":"#0D0D0D"}}>
                  ...
                  <span style={{...COL,color:r.goals>0?"#E8A020":"#2e2e2e",fontWeight:r.goals>0?800:400}}>{r.goals||"–"}</span>
                  <span style={{...COL,color:r.gkPeriods>0?"#38bdf8":"#2e2e2e",fontWeight:r.gkPeriods>0?700:400}}>{r.gkPeriods||"–"}</span>
                  <span style={{...COL,color:r.potm>0?"#fcd34d":"#2e2e2e",fontWeight:r.potm>0?700:400}}>{r.potm||"–"}</span>
                </div>
              ))}
            </>
        }
    ...
    <div style={S.page}>
      <div style={{...S.card,maxWidth:440}}>
        ...
        {games.slice().reverse().map(g=>{
          ...
          const resColor=res==="W"?"#E8A020":res==="L"?"#ef4444":"#f59e0b";
          return (
...
function SeasonHubScreen({ games, onBack, onOpenGame, onDeleteGame }) {
  ...
  if (subScreen === 'log') {
    ...
    <div style={S.page}>
      <div style={{ ...S.card, padding:0, display:'flex', flexDirection:'column', maxHeight:'calc(100vh - max(env(safe-area-inset-top),16px) - max(env(safe-area-inset-bottom),16px) - 32px)' }}>
        <div style={{ padding:'14px 16px 10px', borderBottom:'1px solid #2a2000', flexShrink:0 }}>
          <h2 style={{ margin:'4px 0 0', fontSize:18, fontWeight:800, color:'#f1f5f9' }}>📋 Game Log</h2>
        ...
        <div style={{borderTop:"1px solid #2a2000",padding:"10px 14px",paddingBottom:"max(10px,env(safe-area-inset-bottom))",flexShrink:0}}><button onClick={()=>setSubScreen(null)} style={S.backBtn}>← Season</button></div>
      </div>
    </div>
  );}
  ...
  if (subScreen === 'fixtures') return (
    <div style={S.page}>
      <div style={{ ...S.card, padding:0, display:'flex', flexDirection:'column', maxHeight:'calc(100vh - max(env(safe-area-inset-top),16px) - max(env(safe-area-inset-bottom),16px) - 32px)' }}>
        <div style={{ padding:'14px 16px 10px', borderBottom:'1px solid #2a2000', flexShrink:0 }}>
          <h2 style={{ margin:'4px 0 0', fontSize:18, fontWeight:800, color:'#f1f5f9' }}>📅 Fixtures</h2>
        </div>
  ...
  if (subScreen === 'ladder') return (
    <div style={S.page}>
      <div style={{ ...S.card, padding:0, display:'flex', flexDirection:'column', maxHeight:'calc(100vh - max(env(safe-area-inset-top),16px) - max(env(safe-area-inset-bottom),16px) - 32px)' }}>
        <div style={{ padding:'14px 16px 10px', borderBottom:'1px solid #2a2000', flexShrink:0 }}>
          <h2 style={{ margin:'4px 0 0', fontSize:18, fontWeight:800, color:'#f1f5f9' }}>🏅 Ladder</h2>
        </div>
  ...
  if (subScreen === 'teams') return (
    <div style={S.page}>
      <div style={{ ...S.card, padding:0, display:'flex', flexDirection:'column', maxHeight:'calc(100vh - max(env(safe-area-inset-top),16px) - max(env(safe-area-inset-bottom),16px) - 32px)' }}>
        <div style={{ padding:'14px 16px 10px', borderBottom:'1px solid #2a2000', flexShrink:0 }}>
          <h2 style={{ margin:'4px 0 0', fontSize:18, fontWeight:800, color:'#f1f5f9' }}>👥 Teams</h2>
        </div>
        <div style={{ overflowY:'auto', flex:1, padding:'14px 16px 40px' }}>
          <select value={selectedTeam} onChange={e => selectTeam(e.target.value)}
            style={{ width:'100%', padding:'8px 10px', borderRadius:8, background:'#0D0D0D', color:'#f1f5f9', border:'1px solid #2e2e2e', fontSize:13, marginBottom:14 }}>
          ...
          {selectedTeam && (
            <>
              ...
              <div style={{ marginTop:16, borderTop:'1px solid #2a2000', paddingTop:16 }}>
                <div style={{ fontSize:11, fontWeight:700, color:'#64748b', textTransform:'uppercase', letterSpacing:1, marginBottom:8 }}>📝 Coach Notes</div>
                <textarea value={noteText} onChange={e => setNoteText(e.target.value)}
                  placeholder="Add scouting notes — included in the AI scout report above."
                  style={{ width:'100%', minHeight:90, padding:'10px 12px', background:'#0D0D0D', border:'1px solid #2e2e2e', borderRadius:10, color:'#f1f5f9', fontSize:13, lineHeight:1.6, resize:'vertical', fontFamily:'inherit', outline:'none' }}
                />
    ...
    <div style={S.page}>
      <div style={S.card}>
        ...
        <div style={S.tileGrid}>
          <button style={{ ...S.tile, background:'#0D0D0D', border:'1px solid #2a2000' }} onClick={() => setSubScreen('log')}>
            <span style={S.tileIcon}>📋</span>
            <span style={S.tileLbl}>Game Log</span>
            {games.length > 0 && <span style={{ fontSize:10, color:'#64748b' }}>{games.length} game{games.length!==1?'s':''}</span>}
          </button>
          <button style={{ ...S.tile, background:'#0D0D0D', border:'1px solid #2a2000' }} onClick={() => setSubScreen('fixtures')}>
            <span style={S.tileIcon}>📅</span>
            <span style={S.tileLbl}>Fixtures</span>
          </button>
          <button style={{ ...S.tile, background:'#0D0D0D', border:'1px solid #2a2000' }} onClick={() => setSubScreen('ladder')}>
            <span style={S.tileIcon}>🏅</span>
            <span style={S.tileLbl}>Ladder</span>
          </button>
          <button style={{ ...S.tile, background:'#0D0D0D', border:'1px solid #2a2000' }} onClick={() => setSubScreen('teams')}>
            <span style={S.tileIcon}>👥</span>
            <span style={S.tileLbl}>Teams</span>
          </button>
        </div>

        <div style={{borderTop:"1px solid #2a2000",padding:"10px 14px",paddingBottom:"max(10px,env(safe-area-inset-bottom))",flexShrink:0}}><button onClick={onBack} style={S.backBtn}>← Menu</button></div>
      </div>
    </div>
  );
}
  ...
  return (
    <div style={S.page}>
      <div style={S.card}>
        ...
        <div style={S.tileGrid}>
          <button style={{...S.tile, background:'#2a2000', border:'1px solid #E8A02033'}} onClick={onViewStats}>
            <span style={S.tileIcon}>📊</span><span style={S.tileLbl}>Player Stats</span>
          </button>
          <button style={{...S.tile, background:'#1e1e1e', border:'1px solid #2e2e2e'}} onClick={onManageSquad}>
...
function MiniPitch({ slots, positions }) {
  ...
  return (
    <div style={{position:"relative",width:"100%",aspectRatio:"0.68",borderRadius:12,overflow:"hidden",border:"2px solid #E8A02044",background:"linear-gradient(180deg,#8a5000 0%,#7a4500 50%,#8a5000 100%)"}}>
      ...
      {positions.map(pos=>{
        ...
        return (
          <div key={pos.id} style={{position:"absolute",left:pos.left,top:pos.top,transform:`translateX(${pos.tx})`,display:"flex",flexDirection:"column",alignItems:"center",gap:2,zIndex:1}}>
            {name
              ? <div style={{background:"#2a2000",borderRadius:6,padding:"3px 6px",fontSize:9,fontWeight:700,color:"#fff",textAlign:"center",maxWidth:58,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap",border:"1px solid #3b82f644"}}>{name}</div>
    ...
    <div style={S.page}>
      <div style={{...S.card,maxWidth:460}}>
        ...
        {view==="summary" && <>
          ...
          {game.voiceNotes && (
            <div style={{background:"#0c1a0c",border:"1px solid #E8A02033",borderRadius:10,padding:"10px 14px",marginBottom:10}}>
        ...
        {view==="events" && (() => {
          ...
          return (
            <div>
              ...
              {all.length>0 && (
                <div style={{marginBottom:16}}>
                  <div style={{display:"flex",justifyContent:"space-between",marginBottom:8}}>
                    ...
                    <span style={{fontSize:11,color:"#64748b"}}>Stats</span>
                  ...
                  {MATCH_EVENT_RULES.filter(r=>r.type!=='NOTE').map(rule=>{
                    ...
                    return (<div key={rule.type} style={{marginBottom:10}}>
                      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:3}}>
                        ...
                        <span style={{fontSize:11,color:"#94a3b8"}}>{rule.icon} {rule.label}</span>
                        <span style={{fontSize:13,fontWeight:800,color:"#fca5a5",minWidth:20,textAlign:"right"}}>{them}</span>
                      </div>
                      <div style={{position:"relative",height:5,background:"#1e1e1e",borderRadius:3}}>
                        <div style={{position:"absolute",right:"50%",top:0,height:"100%",width:(us/maxSt*50)+"%",background:"#E8A020",borderRadius:"3px 0 0 3px"}}/>
                        <div style={{position:"absolute",left:"50%",top:0,height:"100%",width:(them/maxSt*50)+"%",background:"#ef4444",borderRadius:"0 3px 3px 0"}}/>
                        <div style={{position:"absolute",left:"50%",top:-2,width:1,height:9,background:"#475569"}}/>
                      </div>
                    </div>);
                  })}
                </div>
              ...
              {/* Timeline */}
              {all.length>0 && <div style={{fontSize:11,fontWeight:700,color:"#64748b",textTransform:"uppercase",letterSpacing:1,marginBottom:8}}>Timeline</div>}
              {all.map(ev=>{
                ...
                return (<div key={ev.id} style={{background:"#0D0D0D",borderLeft:"3px solid "+rule.color,borderRadius:"0 8px 8px 0",padding:"7px 10px",marginBottom:5,display:"flex",alignItems:"flex-start",gap:8}}>
                  <div style={{fontSize:10,color:"#64748b",fontWeight:700,minWidth:28,paddingTop:2}}>{ev.minute}'</div>
                  <div style={{fontSize:15}}>{rule.icon}</div>
                  <div style={{flex:1}}>
                    <div style={{display:"flex",alignItems:"center",gap:5,flexWrap:"wrap"}}>
                      <span style={{fontSize:12,fontWeight:700,color:rule.color}}>{rule.label}</span>
                      {ev.team&&<span style={{fontSize:10,color:ev.team==="us"?"#FCD34D":"#fca5a5",background:ev.team==="us"?"#1a120033":"#7f1d1d33",borderRadius:4,padding:"1px 5px"}}>{ev.team==="us"?"Us":game.opponent||"Them"}</span>}
                    </div>
                    {ev.player&&<div style={{fontSize:11,color:"#94a3b8"}}>👤 {ev.player}</div>}
                    {ev.notes&&<div style={{fontSize:11,color:"#94a3b8"}}>{ev.notes}</div>}
                  </div>
                </div>);
              })}
            </div>
      ...
      <div style={S.card}>
        ...
        {isNewGame && (<>
          <div style={{marginTop:8}}>
            ...
            <div style={{display:"flex",gap:6}}>
              ...
              {opponent.trim() && onViewOpponent && (
                <button style={{...S.btnAdd,background:"#2a2000",border:"1px solid #E8A02033",fontSize:11,padding:"9px 10px",whiteSpace:"nowrap"}} onClick={()=>onViewOpponent(opponent.trim())}>🔍 Scout</button>
              )}
            </div>
          ...
          <div style={S.configBox}>
            ...
            <div style={{fontSize:11,color:"#E8A020",textAlign:"center"}}>= {pmDisplay} min per period</div>
          </div>
        ...
        {isNewGame && <div style={{borderTop:"1px solid #2a2000",paddingTop:10,marginTop:4}}><button style={S.btnGhost} onClick={onBack}>← Back</button></div>}
      </div>
    </div>
  );
}
...
function Token({ name, color, selected, onTap, onDragStart }) {
  const bg=color||(selected?"#7c3aed":"#2a2000");
  return (
...
const MATCH_EVENT_RULES = [
  ...
  { type:"NOTE",         icon:"📝", color:"#94a3b8", label:"Note"             },
];
  ...
  return (
    <div style={{...S.wrap,padding:0,background:BG}}>
      ...
      <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',padding:'10px 16px',background:BG,borderBottom:'1px solid '+CARD,flexShrink:0,width:'100%',maxWidth:500}}>
        <button style={{...S.btnGhost,borderColor:'#2a1f00',color:'#8a6000'}} onClick={onBack}>← Pitch</button>
        <div style={{fontSize:14,fontWeight:800,color:'#f1f5f9'}}>{usGoalsLen} – {themGoalsLen} <span style={{fontSize:11,color:'#8a6000',fontWeight:400}}>{mm}:{ss}</span></div>
        <div style={{width:60}}/>
      </div>
      ...
      <div style={{flex:1,overflowY:'auto',padding:'16px',background:BG,width:'100%',maxWidth:500,boxSizing:'border-box'}}>
        ...
        {evTab==='record' && (
          <div>
            {pendingType && (()=>{
              ...
              return (
                <div style={{background:CARD,border:'2px solid '+rule.color+'55',borderRadius:12,padding:14,marginBottom:14}}>
                  ...
                  {pendingType==='NOTE' ? (
                    <div>
                      <textarea value={noteText} onChange={e=>setNoteText(e.target.value)} rows={2} placeholder="What happened?" style={{...S.inp,resize:'none',width:'100%',marginBottom:8,fontSize:13,background:'#0d200d',borderColor:'#2a1f00'}}/>
                      ...
                      <button onClick={()=>setPendingType(null)} style={{width:'100%',padding:'9px',borderRadius:9,border:'1px solid #2a1f00',background:'transparent',color:'#8a6000',fontSize:12,cursor:'pointer'}}>Cancel</button>
                    </div>
                  ) : (
                    <div>
                      <div style={{display:'flex',gap:8,marginBottom:8}}>
            ...
            {statusMsg&&<div style={{background:'#1a1200',border:'1px solid #E8A02044',borderRadius:9,padding:'9px 12px',marginBottom:12}}><span style={{fontSize:13,color:'#FCD34D',fontWeight:600}}>{statusMsg}</span></div>}
            ...
            <div style={{borderTop:'1px solid '+CARD,paddingTop:14}}>
              <div style={{fontSize:11,fontWeight:700,color:'#8a6000',textTransform:'uppercase',letterSpacing:1,marginBottom:8}}>🤖 Describe what happened</div>
              <textarea value={aiText} onChange={e=>setAiText(e.target.value)} rows={3}
                placeholder='e.g. "Corner for us, keeper tipped it over"'
                style={{...S.inp,resize:'none',width:'100%',lineHeight:1.6,fontSize:13,background:'#0d200d',borderColor:'#2a1f00'}}/>
        ...
        {evTab==='stats' && (
          <div>
            ...
            <div style={{background:CARD,borderRadius:12,padding:'20px 16px',marginBottom:20,textAlign:'center'}}>
              <div style={{fontSize:11,fontWeight:700,color:'#8a6000',letterSpacing:2,textTransform:'uppercase',marginBottom:10}}>FINAL SCORE</div>
              <div style={{fontSize:52,fontWeight:900,color:'#F5D53F',lineHeight:1,marginBottom:12}}>{usGoalsLen} – {themGoalsLen}</div>
              <div style={{display:'flex',justifyContent:'space-between'}}>
                <span style={{fontSize:14,fontWeight:700,color:'#f1f5f9'}}>{OUR}</span>
                <span style={{fontSize:14,fontWeight:700,color:'#f1f5f9'}}>{THEM}</span>
              </div>
            </div>
                ...
                return (
                  <div key={rule.type} style={{marginBottom:14}}>
                    ...
                    <div style={{height:4,background:BORDER,borderRadius:2,position:'relative'}}>
                      ...
                      <div style={{position:'absolute',left:'50%',top:0,height:'100%',width:1,background:'#2a1f00'}}/>
                  ...
                  <div style={{background:CARD,borderRadius:12,overflow:'hidden'}}>
                    {totals.map(({rule,count},i)=>(
                      <div key={rule.type} style={{display:'flex',alignItems:'center',gap:12,padding:'13px 16px',borderBottom:i<totals.length-1?'1px solid '+BORDER:'none'}}>
                        ...
                        <span style={{fontSize:15,fontWeight:700,color:'#f1f5f9'}}>{count}</span>
                      </div>
                    ))}
                  </div>
                </div>
        ...
        {evTab==='export' && (
          <div>
            ...
            <div style={{background:CARD,borderRadius:12,padding:12,marginBottom:20,display:'flex',flexDirection:'column',gap:8}}>
              ...
              <button onClick={exportJSON} style={{background:'#2d5a2d',border:'none',borderRadius:10,padding:'16px',fontSize:14,fontWeight:700,color:'#f1f5f9',cursor:'pointer',width:'100%'}}>
...
function MatchScreen({ half1, config, squad, opponent, linkedFixKey, fixIsHome, onSaveGame, onExit }) {
  ...
  const ringColor=pct>0.4?"#E8A020":pct>0.15?"#f59e0b":"#ef4444";
    ...
    <div style={S.wrap} onTouchStart={onTouchStart} onTouchEnd={onTouchEnd}>
      ...
      <div style={S.header}>
        <div style={{display:"flex",alignItems:"center",gap:6}}>
          ...
          <div style={{display:"flex",flexDirection:"column",gap:3}}>
            <button style={{...S.btnSm,background:running?"#ef4444":"#E8A020"}} onClick={toggleRun}>{running?"⏸":"▶"}</button>
            {alarmed
              ?<button style={{...S.btnSm,background:"#CA8A04"}} onClick={goNextPeriod}>Next→</button>
              :<button style={{...S.btnSm,background:"#475569"}} onClick={resetPeriodTimer}>↺</button>}
          </div>
        </div>
        <button style={S.micBtn} onClick={()=>setEventsView(true)}>
          📊<span style={{fontSize:9,marginTop:2,color:"#94a3b8"}}>Events</span>
        </button>
      </div>
      ...
      <div style={S.mainRow}>
        ...
        <div style={S.bench} onDragOver={e=>e.preventDefault()} onDrop={e=>{e.preventDefault();handleDrop("bench");}} onClick={()=>{if(selected)handleTap(null,true,null);}}>
          ...
          {offSummary.length===0&&<div style={{fontSize:9,color:"#2e2e2e",textAlign:"center"}}>none yet</div>}
      ...
      <div style={{display:"flex",gap:8,width:"100%",maxWidth:500,marginTop:8}}>
        ...
        <button style={S.btnNotes} onClick={toggleVoice}
          ...
          style={{...S.btnNotes,flex:0,width:52,paddingLeft:0,paddingRight:0,flexDirection:"column",gap:2,
            background:isVoiceRec?(voicePulse?"#7f1d1d":"#450a0a"):"#1e1e1e",
            border:isVoiceRec?"1px solid #ef4444":"1px solid #2e2e2e",
      ...
      <div style={{background:CARD,borderRadius:12,padding:'14px 16px',marginBottom:10}}>
        ...
        <div style={{display:'flex',gap:8}}>
          <button onClick={()=>{setExportPending({type,fname:defaultName});setFilename(defaultName);}}
            style={{flex:1,padding:'11px',borderRadius:9,border:'none',background:'#CA8A04',color:'#fff',fontWeight:700,fontSize:13,cursor:'pointer'}}>
    ...
    <div style={{...S.wrap,background:'#0D0D0D'}}>
      ...
      {exportPending&&(
        <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.7)',display:'flex',alignItems:'center',justifyContent:'center',zIndex:9999,padding:24}}>
          <div style={{background:'#1e1e1e',borderRadius:16,padding:24,width:'100%',maxWidth:380}}>
            ...
            <div style={{display:'flex',alignItems:'center',background:'#0D0D0D',border:'1px solid #2e2e2e',borderRadius:9,padding:'10px 12px',marginBottom:16,gap:4}}>
            ...
            <div style={{display:'flex',gap:8}}>
              <button onClick={doExport} style={{flex:2,padding:'12px',borderRadius:9,border:'none',background:'#CA8A04',color:'#fff',fontWeight:700,fontSize:14,cursor:'pointer'}}>
                Export
              </button>
              <button onClick={()=>{setExportPending(null);setFilename('');}} style={{flex:1,padding:'12px',borderRadius:9,border:'1px solid #2e2e2e',background:'transparent',color:'#94a3b8',fontWeight:700,fontSize:14,cursor:'pointer'}}>
                Cancel
              </button>
            </div>
          </div>
      ...
      <h2 style={{margin:"0 0 16px",fontSize:18,fontWeight:800,color:'#f1f5f9'}}>Import / Export</h2>

      {status&&<div style={{background:statusOk?'#1a1200':'#3a0b0b',border:'1px solid '+(statusOk?'#E8A02044':'#ef444444'),borderRadius:9,padding:'10px 14px',marginBottom:14,fontSize:13,color:statusOk?'#FCD34D':'#fca5a5',fontWeight:600,flexShrink:0}}>{status}</div>}
        ...
        <div style={{background:'#3a0b0b',border:'1px solid #ef444433',borderRadius:12,padding:'14px 16px',marginTop:4}}>
          ...
          {!confirmReset
            ...
            :<div style={{display:'flex',gap:8}}>
              ...
              <button onClick={()=>setConfirmReset(false)} style={{flex:1,padding:'11px',borderRadius:9,border:'1px solid #2e2e2e',background:'transparent',color:'#94a3b8',fontWeight:700,fontSize:13,cursor:'pointer'}}>Cancel</button>
            </div>
          }
        </div>
        <div style={{borderTop:"1px solid #2a2000",paddingTop:10,marginTop:4}}>
...
function BottomNav({ activeTab, onTab }) {
  ...
  return (
    <nav style={{
      ...
      background:'#0d1526', borderTop:'1px solid #2a2000',
      ...
      {tabs.map(t => {
        ...
        return (
          <button key={t.id} onClick={() => onTab(t.id)} style={{
            ...
            padding:'4px 8px', color: active ? '#E8A020' : '#64748b',
            minWidth:56,
          }}>
            <span style={{display:'flex', alignItems:'center', justifyContent:'center', width:22, height:22}}>
              {React.cloneElement(t.svg, { stroke: active ? '#E8A020' : '#64748b' })}
            </span>
    ...
    <div style={{ minHeight:'100vh', background:'#0D0D0D', paddingBottom:80, paddingTop:'max(env(safe-area-inset-top),0px)' }}>
      ...
      <div style={{ padding:'10px 20px 0', display:'flex', alignItems:'center', justifyContent:'space-between' }}>
        <div style={{ display:'flex', alignItems:'center', gap:8 }}>
          <div style={{
            ...
            background:'linear-gradient(135deg,#7a4500,#E8A020)',
            display:'flex', alignItems:'center', justifyContent:'center',
            fontSize:14, fontWeight:900, color:'#fff',
          }}>K</div>
          <span style={{ fontSize:20, fontWeight:900, color:'#E8A020', letterSpacing:2 }}>KHULA</span>
        </div>
        <span style={{ fontSize:9, fontWeight:700, color:'#64748b', letterSpacing:1.5 }}>PLAY · CONNECT · ELEVATE</span>
      </div>
      ...
      <div style={{ padding:'14px 20px 16px', display:'flex', alignItems:'center', justifyContent:'space-between' }}>
        <div style={{ display:'flex', alignItems:'center', gap:12 }}>
          ...
          <div>
            <div style={{ fontSize:12, color:'#64748b', fontWeight:500 }}>{getGreeting()},</div>
            <div style={{ fontSize:18, fontWeight:800, color:'#f1f5f9', lineHeight:1.2 }}>{coachName} 👋</div>
            <div style={{ fontSize:11, color:'#64748b', marginTop:2 }}>⚽ {teamName}</div>
          </div>
        </div>
        <button onClick={onGoTeam} style={{ background:'none', border:'1px solid #2a2000', borderRadius:10, padding:'7px 10px', cursor:'pointer', color:'#64748b' }}>
      ...
      {nextFix ? (
        <div onClick={onGoSeason} style={{
          ...
          background:'linear-gradient(135deg,#4a2800 0%,#7a4500 60%,#8a5000 100%)',
          border:'1px solid #E8A02044', cursor:'pointer',
        }}>
        ...
        <div style={{ margin:'0 16px 16px', borderRadius:16, background:'#1e1e1e', border:'1px solid #2a2000', padding:'20px 18px', textAlign:'center' }}>
          <div style={{ fontSize:28, marginBottom:8 }}>📅</div>
          <div style={{ fontSize:14, color:'#64748b' }}>No upcoming fixtures</div>
          <div style={{ fontSize:12, color:'#475569', marginTop:4 }}>Import your fixture schedule in More → Import/Export</div>
        </div>
      )}
      ...
      {played > 0 && (
        <div style={{ margin:'0 16px 16px' }}>
          <div style={{ fontSize:10, fontWeight:700, color:'#64748b', letterSpacing:1.5, textTransform:'uppercase', marginBottom:8 }}>This season</div>
          <div style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:8 }}>
            {[['Played', played, '#f1f5f9'], ['Won', wins, '#E8A020'], ['Drawn', draws, '#f59e0b'], ['Lost', losses, '#ef4444']].map(([label, val, color]) => (
              <div key={label} style={{ background:'#181818', borderRadius:12, padding:'12px 8px', textAlign:'center', border:'1px solid #2a2000' }}>
      ...
      {/* ── Quick Actions ── */}
      <div style={{ margin:'0 16px 16px' }}>
        <div style={{ fontSize:10, fontWeight:700, color:'#64748b', letterSpacing:1.5, textTransform:'uppercase', marginBottom:8 }}>Quick actions</div>
        <div style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:10 }}>
          {[
            ...
            <button key={item.label} onClick={item.action} style={{
              background: item.highlight ? '#E8A020' : '#181818',
              border: item.highlight ? 'none' : '1px solid #2a2000',
              borderRadius:14, padding:'14px 4px 10px',
              display:'flex', flexDirection:'column', alignItems:'center', gap:6,
              cursor:'pointer',
            }}>
              <span style={{ fontSize:24 }}>{item.icon}</span>
              <span style={{ fontSize:11, fontWeight:700, color: item.highlight ? '#0D0D0D' : '#f1f5f9', lineHeight:1.2 }}>{item.label}</span>
            </button>
          ))}
        </div>
      </div>
      ...
      {lastRes && (
        <div style={{ margin:'0 16px 0' }}>
          <div style={{ fontSize:10, fontWeight:700, color:'#64748b', letterSpacing:1.5, textTransform:'uppercase', marginBottom:8 }}>Last result</div>
          <div style={{ background:'#181818', borderRadius:14, padding:'14px 16px', border:'1px solid #2a2000', display:'flex', alignItems:'center', gap:12 }}>
            {(() => {
              const res = lastRes.us > lastRes.them ? 'W' : lastRes.us < lastRes.them ? 'L' : 'D';
              const col = res==='W' ? '#E8A020' : res==='L' ? '#ef4444' : '#f59e0b';
              return <>
                ...
                <div style={{ flex:1 }}>
                  <div style={{ fontSize:14, fontWeight:700, color:'#f1f5f9' }}>vs {lastRes.opponent}</div>
                  <div style={{ fontSize:11, color:'#64748b', marginTop:2 }}>{lastRes.dateStr || ''}{lastRes.round ? ' · ' + lastRes.round : ''}</div>
                </div>
                <div style={{ fontSize:22, fontWeight:800, color:'#f1f5f9' }}>{lastRes.us}–{lastRes.them}</div>
              </>;
            })()}
          </div>
        </div>
  ...
  return (
    <div style={{ minHeight:'100vh', background:'#0D0D0D', paddingBottom:90, paddingTop:'max(env(safe-area-inset-top),16px)' }}>
      <div style={{ padding:'20px 20px 16px' }}>
        <h1 style={{ margin:0, fontSize:24, fontWeight:800, color:'#f1f5f9' }}>More</h1>
      </div>
      <div style={{ padding:'0 16px', display:'flex', flexDirection:'column', gap:10 }}>
        {items.map(item => (
          <button key={item.label} onClick={item.action} style={{
            background:'#181818', border:'1px solid #2a2000', borderRadius:14,
...
const S = {
  ...
  card:         { background:"#181818", border:"1px solid #2a2000", borderRadius:16, padding:"24px 20px", width:"100%", maxWidth:DEVICE.cardMax, display:"flex", flexDirection:"column", gap:12 },
  pill:         { alignSelf:"flex-start", background:"#E8A02022", color:"#E8A020", border:"1px solid #E8A02044", borderRadius:20, fontSize:11, fontWeight:700, padding:"3px 12px", letterSpacing:2 },
  ...
  inp:          { flex:1, background:"#0D0D0D", border:"1px solid #2e2e2e", borderRadius:8, color:"#f1f5f9", fontSize:14, padding:"9px 12px", outline:"none", fontFamily:"inherit" },
  btnAdd:       { background:"#CA8A04", color:"#fff", border:"none", borderRadius:8, padding:"9px 16px", fontWeight:700, fontSize:14, cursor:"pointer" },
  btnGreen:     { background:"#E8A020", color:"#0D0D0D", border:"none", borderRadius:10, padding:"12px 0", fontWeight:800, fontSize:14, cursor:"pointer" },
  btnBlue:      { background:"#CA8A04", color:"#fff", border:"none", borderRadius:10, padding:"12px 0", fontWeight:800, fontSize:14, cursor:"pointer", display:"flex", alignItems:"center", justifyContent:"center", gap:6 },
  btnDark:      { background:"#0D0D0D", color:"#cbd5e1", border:"1px solid #2e2e2e", borderRadius:10, padding:"12px 0", fontWeight:700, fontSize:14, cursor:"pointer" },
  btnCancel:    { background:"#1e1e1e", color:"#94a3b8", border:"1px solid #2e2e2e", borderRadius:10, padding:"12px 16px", fontWeight:700, fontSize:14, cursor:"pointer" },
  btnGhost:     { background:"none", border:"1px solid #2e2e2e", color:"#94a3b8", borderRadius:8, padding:"6px 10px", fontSize:14, cursor:"pointer", alignSelf:"flex-start" },
  backBtn:      { background:"none", border:"1px solid #2e2e2e", color:"#94a3b8", borderRadius:8, padding:"6px 10px", fontSize:13, cursor:"pointer" },
  ...
  scoreTile:    { background:"#0D0D0D", borderRadius:12, padding:"14px 16px", border:"1px solid #2a2000" },
  ...
  scorerBox:    { background:"#0D0D0D", borderRadius:10, padding:"10px 12px", border:"1px solid #E8A02022", display:"flex", flexDirection:"column", gap:6 },
  ...
  posSel:       { background:"#1e1e1e", border:"1px solid #2e2e2e", borderRadius:6, color:"#94a3b8", fontSize:11, padding:"4px 4px", outline:"none", fontFamily:"inherit", maxWidth:96 },
  empty:        { fontSize:13, color:"#2e2e2e", padding:"12px 0", textAlign:"center" },
  ...
  stepBtn:      { background:"#1e1e1e", border:"1px solid #2e2e2e", color:"#f1f5f9", borderRadius:6, width:28, height:28, fontSize:16, cursor:"pointer", display:"flex", alignItems:"center", justifyContent:"center" },
  ...
  sel:          { flex:1, background:"#0D0D0D", border:"1px solid #2e2e2e", borderRadius:8, color:"#f1f5f9", fontSize:13, padding:"8px 10px", outline:"none", fontFamily:"inherit" },
  benchPreview: { background:"#0D0D0D", borderRadius:8, padding:"8px 12px", fontSize:12, color:"#64748b" },
  benchPreviewHd:{ fontWeight:700, color:"#475569" },
  wrap:         { minHeight:"100vh", background:"#0D0D0D", color:"#f1f5f9", fontFamily:"'Inter',system-ui,sans-serif", display:"flex", flexDirection:"column", alignItems:"center", padding:"10px 8px 24px", paddingTop:"max(env(safe-area-inset-top),10px)" },
  ...
  gameClock:    { display:"flex", flexDirection:"column", alignItems:"center", background:"#1e1e1e", borderRadius:8, padding:"4px 10px", border:"1px solid #2e2e2e" },
  ...
  micBtn:       { display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", width:52, height:52, borderRadius:12, border:"1px solid #2e2e2e", fontSize:22, cursor:"pointer", flexShrink:0, background:"#1e1e1e", transition:"background 0.2s" },
  ...
  tab:          { background:"#1e1e1e", border:"1px solid #2e2e2e", color:"#94a3b8", borderRadius:8, padding:"5px 9px", fontSize:11, fontWeight:600, cursor:"pointer" },
  tabOn:        { background:"#CA8A04", border:"1px solid #3b82f6", color:"#fff" },
  ...
  pitchOuter:   { flex:1, borderRadius:12, overflow:"hidden", border:"2px solid #E8A02044", background:"linear-gradient(180deg,#8a5000 0%,#7a4500 50%,#8a5000 100%)", position:"relative", aspectRatio:"0.68", touchAction:"none" },
  ...
  bench:        { width:88, borderRadius:12, border:"1px solid #2e2e2e", background:"#1a2236", padding:"10px 6px", display:"flex", flexDirection:"column", alignItems:"center", minHeight:280 },
  benchHd:      { fontSize:9, fontWeight:800, color:"#64748b", letterSpacing:1.5, marginBottom:8 },
  benchEm:      { fontSize:12, color:"#2e2e2e", marginTop:8 },
  benchDiv:     { width:"100%", height:1, background:"#2e2e2e", margin:"8px 0" },
  hint:         { marginTop:6, fontSize:10, color:"#475569", textAlign:"center", marginBottom:12 },
  goalSection:  { width:"100%", maxWidth:500, background:"#1e1e1e", borderRadius:12, padding:"12px 14px", marginTop:8, display:"flex", flexDirection:"column", gap:8 },
  ...
  scoreDash:    { fontSize:20, color:"#2e2e2e" },
  btnGoalUs:    { background:"#7a4500", color:"#FDE68A", border:"1px solid #E8A02044", borderRadius:7, padding:"6px 10px", fontSize:11, fontWeight:700, cursor:"pointer" },
  ...
  btnNotes:     { flex:1, background:"#1e1e1e", border:"1px solid #2e2e2e", color:"#94a3b8", borderRadius:10, padding:"11px 0", fontWeight:700, fontSize:13, cursor:"pointer", display:"flex", alignItems:"center", justifyContent:"center", gap:4 },
  ...
  btnReport:    { flex:1, background:"linear-gradient(135deg,#CA8A04,#7c3aed)", color:"#fff", border:"none", borderRadius:10, padding:"11px 0", fontWeight:800, fontSize:13, cursor:"pointer" },
  halfBar:      { display:"flex", alignItems:"center", justifyContent:"space-between", gap:10, width:"100%", maxWidth:500, marginTop:14, background:"#1e1e1e", borderRadius:12, padding:"8px 12px" },
  halfArrow:    { background:"#0D0D0D", border:"1px solid #2e2e2e", color:"#f1f5f9", borderRadius:8, width:40, height:36, fontSize:18, fontWeight:800, cursor:"pointer" },
  halfBarCenter:{ display:"flex", gap:6 },
  halfPill:     { fontSize:12, fontWeight:700, color:"#94a3b8", background:"#0D0D0D", border:"1px solid #2e2e2e", borderRadius:8, padding:"7px 14px", cursor:"pointer" },
  halfPillOn:   { color:"#f1f5f9", background:"#CA8A04", borderColor:"#3b82f6" },
  swipeHint:    { fontSize:9, color:"#2e2e2e", textAlign:"center", margin:"2px 0 0" },
};
