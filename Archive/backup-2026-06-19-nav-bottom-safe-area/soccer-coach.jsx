    ...
    <div style={S.page}>
      <div style={S.card}>
        ...
        <div style={{ ...S.scoreTile }}>
          {played===0
            ? <div style={{fontSize:13,color:"#475569",textAlign:"center",padding:"8px 0"}}>No games yet — tap New Game to start!</div>
            : <>
                ...
                <div style={{display:"flex",gap:8,marginTop:6,paddingTop:6,borderTop:"1px solid #1e3a5f"}}>
                  <div style={S.scoreTileStat}><div style={{...S.scoreTileNum,fontSize:16,color:"#86efac"}}>{gf}</div><div style={S.scoreTileLbl}>Goals For</div></div>
                  <div style={S.scoreTileStat}><div style={{...S.scoreTileNum,fontSize:16,color:"#fca5a5"}}>{ga}</div><div style={S.scoreTileLbl}>Goals Against</div></div>
                  <div style={S.scoreTileStat}><div style={{...S.scoreTileNum,fontSize:16,color:gd>=0?"#86efac":"#fca5a5"}}>{gd>=0?"+":""}{gd}</div><div style={S.scoreTileLbl}>Goal Diff</div></div>
                </div>
              </>
          }
        </div>
        ...
        <div style={S.tileGrid}>
          ...
          <button style={{...S.tile,background:"#0c4a6e",border:"1px solid #0ea5e933"}} onClick={onViewSeason}>
            <span style={S.tileIcon}>🏆</span><span style={S.tileLbl}>Season</span>
          </button>
...
export default function App() {
  ...
  const [squadBackTo, setSquadBackTo] = useState("menu");

  function saveGame(game){
    const next=[...games,game]; setGames(next); saveGames(next);
    // Write score to fixtureScores so fixtures/ladder/season all see it
    if(game.linkedFixtureKey){
  ...
  return <MatchScreen half1={half1} config={config} squad={squad} opponent={opponent} linkedFixKey={linkedFixKey} fixIsHome={fixIsHome} onSaveGame={saveGame} onExit={()=>setScreen("menu")} />;
}

// ── Styles ────────────────────────────────────────────────────────────────────
const S = {
  page:         { minHeight:"100vh", background:"#0f172a", display:"flex", alignItems:"center", justifyContent:"center", padding:16, paddingTop:"max(env(safe-area-inset-top),16px)", paddingBottom:"max(env(safe-area-inset-bottom),16px)", boxSizing:"border-box" },
  card:         { background:"#1e293b", borderRadius:16, padding:"24px 20px", width:"100%", maxWidth:DEVICE.cardMax, display:"flex", flexDirection:"column", gap:12 },
  ...
  wrap:         { minHeight:"100vh", background:"#0f172a", color:"#f1f5f9", fontFamily:"'Inter',system-ui,sans-serif", display:"flex", flexDirection:"column", alignItems:"center", padding:"10px 8px 24px", paddingTop:"max(env(safe-area-inset-top),10px)" },
